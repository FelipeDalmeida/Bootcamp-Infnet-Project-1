            ...
            <div className={"absolute inset-0 bg-slate-900/25 backdrop-blur-sm transition-opacity opacity-100"}>
                <div className={"fixed inset-0 flex items-start justify-end overflow-y-auto translate-x-0"}>
                    <div className={"h-full w-[min(20rem,calc(100vw-theme(spacing.10)))] bg-white shadow-2xl ring-1 ring-black/10 transition flex flex-col "}>
                        <button className={"md:hidden px-6"}>{<FaTimes className={"text-sky-700 h-10 w-5"} onClick={changeState} />}</button>
                        <div className={"flex flex-col  items-center self-center [&>a]:p-6 [&>a]:w-full [&>a]:text-sky-700 [&>a]:cursor-pointer text-lg [&>a]:text-center"} onClick={changeState}>{anchor}</div>
                        <span className={"border-t border-y-slate-200 flex flex-col self-center [&>a]:p-6 [&>a]:w-full items-center [&>a]:text-sky-700 [&>a]:cursor-pointer text-lg [&>a]:text-center"} onClick={changeState}>{anchor2}</span>
                    </div>
                </div>
            </div>
        </div>
